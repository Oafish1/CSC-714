% GMRES implementation from Wikipedia, modified to support our mapping P
function [x, e] = gmres( b, x, max_iterations, threshold, AP,...
    ba,tb,n_x,n_y,PML,woverc,h)
  m = max_iterations;

  % use x as the initial vector
  r = b - AP(ba,tb,x,n_x,n_y,PML,woverc,h);
  ...
  for k = 1:m
    ...
    % run arnoldi
    [H(1:k+1, k) Q(:, k+1)] = arnoldi(Q, k,AP,ba,tb,n_x,n_y,PML,woverc,h);
...
%                  Arnoldi Function                  %
%----------------------------------------------------%
function [h, q] = arnoldi(Q, k, AP,ba,tb,n_x,n_y,PML,woverc,h)
  q = AP(ba,tb,Q(:,k),n_x,n_y,PML,woverc,h);   % Krylov Vector
